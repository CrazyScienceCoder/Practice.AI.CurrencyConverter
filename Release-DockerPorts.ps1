...
foreach ($entry in $ports) {
    ...
    if (-not $existing) {
        Write-Host "[$port] Not reserved, skipping - $service" -ForegroundColor Gray
    } else {
        $result = netsh int ipv4 delete excludedportrange protocol=tcp startport=$port numberofports=1
        if ($LASTEXITCODE -eq 0) {
            Write-Host "[$port] Released - $service" -ForegroundColor Green
        } else {
            Write-Warning "[$port] Failed to release - $service`n$result"
        }
    }
}
