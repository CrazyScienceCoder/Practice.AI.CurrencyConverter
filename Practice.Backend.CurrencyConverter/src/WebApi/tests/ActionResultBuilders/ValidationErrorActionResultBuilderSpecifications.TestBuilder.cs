    ...
    private class TestBuilder
    {
        ...
        public TestBuilder()
        {
            ...
            _problemDetailsFactoryMock
                ...
                {
                    var validationProblemDetails = new ValidationProblemDetails { Status = status, Title = title, Detail = detail };
                    foreach (var key in msd.Keys)
                    {
                        var errors = msd[key]?.Errors.Select(e => e.ErrorMessage).ToArray() ?? [];
                        validationProblemDetails.Errors[key] = errors;
                    }
                    return validationProblemDetails;
                });
        }

        public ValidationErrorActionResultBuilder Build()
