...
public sealed class ConversionEndpoint(IMediator mediator, IActionResultBuilderFactory factory)
    : WebApiBaseController(factory)
{
    [HttpGet(template: Routes.Endpoints.Conversion)]
    [ProducesResponseType(type: typeof(GetCurrencyConversionQueryResult), statusCode: StatusCodes.Status200OK)]
    [ProducesResponseType(statusCode: StatusCodes.Status400BadRequest)]
    [ProducesResponseType(statusCode: StatusCodes.Status404NotFound)]
    [ProducesResponseType(statusCode: StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> InvokeAsync([FromQuery] ConversionRequest request
